...
final class CachedElement: @unchecked Sendable {
    ...
    func getCurrentFrame() -> CGRect? {
        ...
        guard AXUIElementCopyAttributeValue(axElement, kAXPositionAttribute as CFString, &positionValue)
            == .success,
...
func openApplication(identifier: String) async -> Result<AppInfo, AppError> {
    ...
    if let app = runningApps.first(where: {
        ...
        return .success(
            AppInfo(pid: app.processIdentifier, bundleId: app.bundleIdentifier, name: app.localizedName ?? identifier)
        )
    }
    ...
    do {
        ...
        if let url = workspace.urlForApplication(withBundleIdentifier: identifier) {
            ...
            return .success(
                AppInfo(
                    pid: app.processIdentifier, bundleId: app.bundleIdentifier, name: app.localizedName ?? identifier)
            )
        }
