...
struct ScreenshotResult: Encodable {
    ...
    static func ok(width: Int, height: Int, base64: String) -> ScreenshotResult {
        return ScreenshotResult(success: true, width: width, height: height, base64: base64, error: nil)
    }

    static func fail(_ message: String) -> ScreenshotResult {
        return ScreenshotResult(success: false, width: nil, height: nil, base64: nil, error: message)
    }
}
...
final class ScreenshotController: @unchecked Sendable {
    ...
    func capture(options: ScreenshotOptions = ScreenshotOptions()) -> ScreenshotResult {
        ...
        if let scale = options.scale, scale > 0 && scale < 1.0 {
        ...
        // Convert to data
        let format = options.format?.lowercased() ?? "png"
